...
def main():
    ...
    with console.status("[bold green]Running trade classifier training...") as status:
        # 初始化進度顯示
        console.print("[bold blue]Step 1:[/bold blue] Initializing backtest strategy")
        ...
        console.print("[bold blue]Step 2:[/bold blue] Setting up backtest parameters")
        ...
        console.print("[bold blue]Step 3:[/bold blue] Running backtest simulation")
        backtest.run()
        
        console.print("[bold blue]Step 4:[/bold blue] Preparing training data")
        trades_df = pd.DataFrame(backtest.strategy.trades_history)
        stock_data = backtest.data_cache.get_data()
        
        # 顯示交易資料統計
        trade_stats = Table(title="Trading Data Statistics")
        trade_stats.add_column("Metric", style="cyan")
        trade_stats.add_column("Value", style="magenta")
        trade_stats.add_row("Total Trades", str(len(trades_df)))
        trade_stats.add_row("Date Range", f"{trades_df['timestamp'].min()} to {trades_df['timestamp'].max()}")
        console.print(trade_stats)
        
        console.print("[bold blue]Step 5:[/bold blue] Initializing classifier")
        classifier = TradeClassifier(model_path=f"models/{StrategyConfig.target_stock}.joblib")
        
        console.print("[bold blue]Step 6:[/bold blue] Preparing features and labels")
        features, labels = classifier.prepare_data(trades_df, stock_data)
        
        # 顯示特徵統計
        feature_stats = Table(title="Feature Statistics")
        feature_stats.add_column("Feature Count", style="cyan")
        feature_stats.add_column("Label Classes", style="magenta")
        feature_stats.add_row(str(features.shape[1]), str(len(labels.unique())))
        console.print(feature_stats)
        
        console.print("[bold blue]Step 7:[/bold blue] Training model")
