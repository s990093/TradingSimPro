...
def calculate_returns(buy_signals: np.ndarray, sell_signals: np.ndarray, close_prices: np.ndarray, stop_loss_pct=0.02, take_profit_pct=0.05) -> Tuple[float, np.ndarray]:
    ...
    # Define transaction fees and taxes
    transaction_fee_rate = 0.001425  # 0.1425%
    transaction_tax_rate = 0.003  # 0.3%

    # Allocate space for trades with an additional column for total_return
    trades = np.zeros((len(close_prices), 5))  # Adjust size as needed
    ...
    for i in range(len(close_prices)):
        if buy_signals[i] == 1 and not holding:
            ...
            if current_price <= buy_price * (1 - stop_loss_pct):
                ...
                transaction_cost = sell_price * (transaction_fee_rate + transaction_tax_rate)
                total_return += (sell_price - buy_price - transaction_cost)
                holding = False
                trades[trade_count, 0] = i  # Trade index
        ...
        # Store the total return at each time step in the trades array
        trades[trade_count - 1, 4] = total_return
