...
class ChannelBreakoutStrategy(BaseStrategy):
    ...
    def apply_strategy(self, df):
        ...
        df['breakout_signal'] = np.where(df['Close'] > df['highest_high'].shift(1), self.TradingSignals.BUY, np.where(df['Close'] < df['lowest_low'].shift(1), self.TradingSignals.SELL, self.TradingSignals.HOLD))  # 突破高點買入，跌破低點賣出
